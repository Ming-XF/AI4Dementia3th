...
class LMDA(nn.Module):
    ...
    """
    def __init__(self, node_size, time_series_size, d_model, depth=9, channel_depth1=24, channel_depth2=9, ave_depth=1, avepool=5):
        super(LMDA, self).__init__()
        self.channel_weight = nn.Parameter(torch.randn(depth, d_model, node_size), requires_grad=True)
        ...
        self.time_conv = nn.Sequential(
            ...
            nn.GELU(),
            nn.Dropout(p=0.5),
            nn.Conv2d(channel_depth1, channel_depth1, kernel_size=(1, 75), groups=channel_depth1, bias=False),
            nn.BatchNorm2d(channel_depth1),
            nn.GELU(),
        )
        ...
        self.chanel_conv = nn.Sequential(
            ...
            nn.GELU(),
            nn.Dropout(p=0.5),
            nn.Conv2d(channel_depth2, channel_depth2, kernel_size=(node_size, 1), groups=channel_depth2, bias=False),
            nn.BatchNorm2d(channel_depth2),
            nn.GELU()
        )
        ...
        hidden_size = time_series_size - 75 + 1
        self.depthAttention = EEGDepthAttention(hidden_size, node_size, k=7)
        hidden_size = channel_depth2 * (hidden_size // avepool)
        self.dense = nn.Linear(hidden_size, d_model)
        self.bnorm = nn.BatchNorm1d(d_model)
        self.act = nn.GELU()
        self.drop = nn.Dropout(p=0.5)
        # self.classifier = nn.Linear(hidden_size, config.num_classes)

        # self.loss_fn = nn.CrossEntropyLoss(label_smoothing=config.label_smoothing, weight=torch.tensor(config.class_weight))
    ...
    def forward(self, time_series):
        time_series = time_series.permute(0, 3, 2, 1)
        ...
        out = self.drop(self.act(self.bnorm(self.dense(features))))
...
class VAE(nn.Module):
    def __init__(self, view, channel1=8, channel2=16, channel3=32, d_model=32):
        ...
        self.encoder = nn.Sequential(
            ...
            nn.GELU(),
            nn.Conv1d(channel1, d_model, kernel_size=3, stride=2, padding=1),
        ...
        self.decoder = nn.Sequential(
            ...
            # nn.LeakyReLU(negative_slope=0.33),
            nn.ConvTranspose1d(d_model, channel1, kernel_size=3, stride=2, padding=1, output_padding=1),
            nn.BatchNorm1d(channel1),
            nn.GELU(),
