from .VIBLayers import *
from ..base import BaseConfig, ModelOutputs


class VIBConfig(BaseConfig):
    def __init__(self,
                 node_size,
                 d_model=128,
                 num_classes=2,
        ...
        super(VIBConfig, self).__init__(dropout=dropout)
...
class VIB(nn.Module):
    ...
    """
    def __init__(self, config: VIBConfig):
        super().__init__()
        self.config = config


        self.vae = VAE(view="t", d_model=config.d_model//2)
    ...
    def forward(self, time_series, node_feature, labels, r_mu, r_logvar, train):
        mu, _, kl = self.vae(time_series, None, None)

        a = self.batch_channel_pearson(mu)

        adj = torch.mean(a, dim=1).unsqueeze(1)
        ...
        loss = logit_loss + 0.001 * kl
        

        if train:
            return ModelOutputs(logits=logits, loss=loss), mu
        else:
